  ...
  <Card class="overflow-hidden">
    ...
    <div class="p-4 space-y-4 max-h-[520px] overflow-y-auto">
      <div v-if="templates.length" class="space-y-3">
        <div class="flex items-center justify-between">
          <span class="text-sm font-semibold text-gray-800">Templates premium</span>
        ...
        <div class="relative">
          <div class="grid grid-cols-2 gap-3" :class="!isPremium ? 'opacity-60' : ''">
          <button
            v-for="template in templates"
            :key="template.id"
            type="button"
            class="border rounded-lg p-2 text-left transition shadow-sm hover:shadow-md"
            :class="selectedTemplateId === template.id ? 'border-[#2ECC71] ring-2 ring-emerald-100' : 'border-gray-200'"
            @click="isPremium ? emit('apply-template', template) : emit('upgrade')"
          >
            <div
          ...
          >
            ...
            >
              <Lock class="h-4 w-4" />
              Debloquer les templates premium
            </button>
          </div>
        </div>
      </div>
      <div class="space-y-3">
        ...
        <div class="relative">
          <div class="grid grid-cols-1 gap-3" :class="!isPremium ? 'opacity-60' : ''">
    ...
    <div v-if="variant === 'full'" class="p-4 space-y-3">
      <button
        ...
        :class="!isPremium ? 'opacity-70' : ''"
        @click="isPremium ? emit('download') : emit('upgrade')"
      >
        ...
        <Lock v-if="!isPremium" class="h-4 w-4 text-amber-600" />
      </button>
    </div>
  </Card>
</template>
